...
int main(int argc, char** argv) {
	...
	if (argc != 2) {
		printf("RunCoder15 (C) Feb. 2010, Andrew Polar.\n");
		printf("This is free software under GPL, http://www.gnu.org/copyleft/gpl.html\n\n");
		printf("Usage :   runcoder15 filename\n");
		printf("File will not be destroyed, no data will be saved on hard drive.\n");
		return 0;
	}

	//allocate memory and read data
	int data_size = getFileSize(argv[1]);
	if (data_size < 0) {
